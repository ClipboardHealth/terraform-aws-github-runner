  ...
  description = <<EOT
    multi_runner_config = {
      runner_config: {
        ...
        scale_down_idle_confirmation_seconds: "Number of seconds a runner must consistently report not-busy before scale-down terminates it. Set to at least one scale-down schedule interval to require two observations; a busy reading resets the window. 0 keeps the previous single-reading behavior."
